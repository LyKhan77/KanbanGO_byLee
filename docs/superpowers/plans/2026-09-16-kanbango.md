...
export function load(): KanbanState | null {
  try {
    ...
    if (p?.version !== VERSION || !Array.isArray(p.boards)) return null;
    return p;
  } catch {
    return null;
  }
...
export function importJSON(text: string): Board[] {
  ...
  else if (p && typeof p === 'object' && p.board) list = [p.board];
  else if (p && typeof p === 'object' && Array.isArray(p.boards)) list = p.boards;
...
function normalizeCard(c: any): Card {
  return {
    ...
    labelIds: Array.isArray(c?.labelIds) ? c.labelIds.filter((x: any) => typeof x === 'string') : [],
    dueDate: typeof c?.dueDate === 'string' ? c.dueDate : null,
    subtasks: Array.isArray(c?.subtasks)
      ? c.subtasks.map((s: any) => ({ id: str(s?.id, uid()), title: str(s?.title, ''), done: !!s?.done }))
      : [],
    createdAt: str(c?.createdAt, nowISO()),
    updatedAt: str(c?.updatedAt, nowISO()),
  };
}

function normalizeLabel(l: any): Label {
  return { id: str(l?.id, uid()), name: str(l?.name, 'Label'), tint: isTint(l?.tint) ? l.tint : 'sky' };
}

function normalizeColumn(c: any): Column {
  return {
    id: str(c?.id, uid()),
    title: str(c?.title, 'Column'),
    tint: isTint(c?.tint) ? c.tint : 'sky',
...
function normalizeBoard(b: any): Board {
  if (!b || typeof b !== 'object' || !Array.isArray(b.columns)) throw new Error('Invalid board: missing columns[]');
  return {
    id: str(b.id, uid()),
    name: str(b.name, 'Imported'),
    columns: b.columns.map(normalizeColumn),
    labels: Array.isArray(b.labels) ? b.labels.map(normalizeLabel) : [],
    createdAt: str(b.createdAt, nowISO()),
    updatedAt: str(b.updatedAt, nowISO()),
  };
}
```
...
In `src/stores/kanban.ts`, add `import type { TintKey }` to the types import, then insert these functions in the setup body (before the final `return`) and extend the `return` object with all of them:
...
3. Board: add a column "REVIEW" (sage) from the banner.
